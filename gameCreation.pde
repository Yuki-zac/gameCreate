...
// TicTacToeクラス
class TicTacToe {
  int[][] board = new int[3][3]; // 盤面
  int currentPlayer = 1; // 1: プレイヤー1 (X), 2: プレイヤー2 (O)
  ...
  void display() {
    ...
    // 勝利メッセージの表示
    if (gameOver) {
      displayWinMessage(checkWin());
    }
  }

  void drawX(float x, float y) {
    line(x + 20, y + 20, x + width / 3 - 20, y + height / 3 - 20);
    line(x + 20, y + height / 3 - 20, x + width / 3 - 20, y + 20);
  }

  void drawO(float x, float y) {
    ellipse(x + width / 6, y + height / 6, width / 6 - 40, height / 6 - 40);
  }
  ...
  void displayWinMessage(boolean win) {
    String message;
    if (win) {
      message = "Player " + (currentPlayer == 1 ? "1 (X)" : "2 (O)") + " Wins!";
    } else {
      message = "It's a Draw!";
    }
    fill(0);
...
void setup() {
  size(1000, 1000);
  o = new Othello(false); // オセロゲーム
  t = new TicTacToe(); // TicTacToeゲーム
}
...
void displayGameHub() {
  ...
  drawButton(width / 2 - 75, height / 2 + 125, "TicTacToe");// 〇×ゲーム
  drawButton(width / 2 - 75, height / 2 + 200, "Help"); // ヘルプボタン
}
...
void mousePressed() {
  if (GameSelection == 0) {
    ...
    if (mouseX > width / 2 - 75 && mouseX < width / 2 + 75 && mouseY > height / 2 - 25 && mouseY < height / 2 + 25) {
      GameSelection = 1; // シングルプレイヤーオセロ
      o = new Othello(true); // シングルプレイヤーモード
    }
    // 2人用オセロの選択ボタン
    if (mouseX > width / 2 - 75 && mouseX < width / 2 + 75 && mouseY > height / 2 + 50 && mouseY < height / 2 + 100) {
      GameSelection = 1; // 2人用オセロ
      o = new Othello(false); // 2人用モード
    }
    // TicTacToeの選択ボタン
    if (mouseX > width / 2 - 75 && mouseX < width / 2 + 75 && mouseY > height / 2 + 125 && mouseY < height / 2 + 175) {
      GameSelection = 2; // TicTacToeゲームに変更
      t = new TicTacToe(); // TicTacToeの初期化
    }
    // ヘルプボタン
    if (mouseX > width / 2 - 75 && mouseX < width / 2 + 75 && mouseY > height / 2 + 200 && mouseY < height / 2 + 250) {
      showHelp = !showHelp; // ヘルプの表示/非表示をトグル
    }
  } else if (GameSelection == 1) {
    // オセロの盤面でのクリック処理
    o.mouseJudge(mouseX, mouseY);
  } else if (GameSelection == 2) {
    // TicTacToeの盤面でのクリック処理
